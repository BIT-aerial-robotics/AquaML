...
@configclass
class DDPGCfg:
    """Configuration class for DDPG algorithm"""
...
class DDPGMemory:
    """Replay buffer for DDPG algorithm"""
...
class DDPG(Agent):
    """Deep Deterministic Policy Gradient (DDPG) algorithm implementation for AquaML
    ...
    def __init__(
        ...
        Args:
            models: Dictionary containing policy, target_policy, critic, target_critic models
        ...
        logger.info(f"DDPG initialized with device: {self.device}")
        
        # Models
        self.policy = models.get("policy")
        self.target_policy = models.get("target_policy")
        self.critic = models.get("critic")
        self.target_critic = models.get("target_critic")
        
        # Validate required models
        if self.policy is None:
            raise ValueError("Policy model is required for DDPG")
        if self.critic is None:
            raise ValueError("Critic model is required for DDPG")
        if self.target_policy is None:
            raise ValueError("Target policy model is required for DDPG")
        if self.target_critic is None:
            raise ValueError("Target critic model is required for DDPG")
        ...
        # Setup memory
        self.memory = DDPGMemory(cfg.memory_size, self.device)
        ...
        self.critic_optimizer = torch.optim.Adam(
            self.critic.parameters(), lr=self._critic_learning_rate
        )
        ...
        self.register_checkpoint_module("critic", self.critic)
        self.register_checkpoint_module("target_critic", self.target_critic)
        ...
        logger.info("DDPG agent initialized successfully")
    ...
    def act(
        ...
        if timestep < self._random_timesteps:
            ...
            if self.clip_actions_min is not None and self.clip_actions_max is not None:
                random_actions = torch.rand_like(self.clip_actions_min) * (
    ...
    def _update(self, timestep: int, timesteps: int):
        ...
        for gradient_step in range(self._gradient_steps):
            ...
            with torch.autocast(device_type=self.device.type if hasattr(self.device, 'type') else 'cpu', enabled=self._mixed_precision):
                ...
                with torch.no_grad():
                    ...
                    target_critic_inputs = {**next_states_dict, "taken_actions": next_actions}
                    target_q_outputs = self.target_critic.act(target_critic_inputs)
                    target_q_values = target_q_outputs.get("values", target_q_outputs.get("actions"))
                ...
                # Compute critic loss
                critic_inputs = {**states_dict, "taken_actions": actions_dict}
                critic_outputs = self.critic.act(critic_inputs)
                critic_values = critic_outputs.get("values", critic_outputs.get("actions"))
                
                critic_loss = F.mse_loss(critic_values.squeeze(), target_values)
            ...
            with torch.autocast(device_type=self.device.type if hasattr(self.device, 'type') else 'cpu', enabled=self._mixed_precision):
                ...
                critic_inputs_policy = {**states_dict, "taken_actions": policy_actions}
                critic_outputs_policy = self.critic.act(critic_inputs_policy)
                critic_values_policy = critic_outputs_policy.get("values", critic_outputs_policy.get("actions"))
                
                policy_loss = -critic_values_policy.mean()
            ...
            self._soft_update(self.target_critic, self.critic, self._polyak)
            ...
            self.track_data("Q-network/Q_Max", torch.max(critic_values).item())
            self.track_data("Q-network/Q_Min", torch.min(critic_values).item())
            self.track_data("Q-network/Q_Mean", torch.mean(critic_values).item())
        ...
        logger.debug(f"DDPG update completed at timestep {timestep}")
    ...
    def save(self, path: str):
        ...
        logger.info(f"DDPG model saved to {path}")
    ...
    def load(self, path: str):
        ...
        logger.info(f"DDPG model loaded from {path}")
