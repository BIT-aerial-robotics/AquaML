...
class BaseRLAlgo(BaseAlgo, abc.ABC):
    ...
    def __init__(self, env, rl_io_info: RLIOInfo, name: str, update_interval: int = 0, computer_type: str = 'PC',
    ...
    def create_gaussian_exploration_policy(self):
        ...
        if self.rl_io_info.explore_info == 'self':
            ...
            self.log_std = DataUnit(name=self.name + '_log_std', dtype=np.float32,
                                    shape=self.rl_io_info.actor_out_info['action'].shape,
                                    level=self.level, computer_type=self._computer_type)

            self.log_std.set_value(np.zeros(self.rl_io_info.actor_out_info['action'].shape, dtype=np.float32) - 0.5)
