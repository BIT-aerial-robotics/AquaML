...
class PPOAgent(BaseRLAgent):
    ...
    def init(self):
        ...
        if self.level == 0:
            ...
            # 最小动作参数
            self.min_action_param = tf.Variable(-7.5, trainable=True, dtype=tf.float32, name='min_action_param')
        ...
        with tf.GradientTape() as tape:
            ...
            total_loss = -actor_surrogate_loss - entropy_coef * entropy_loss + vf_coef * critic_loss + tf.exp(
                tf.clip_by_value(self.min_action_param, clip_value_min=-9.0,
                                 clip_value_max=-6.8)) * actor_action_penalty_loss
    ...
    def train_min_action(self, advantage, action):
        ...
        min_action = tf.clip_by_value(self.min_action_param, clip_value_max=-6.8,clip_value_min=-9.0)

        self.min_action_param.assign(min_action)
