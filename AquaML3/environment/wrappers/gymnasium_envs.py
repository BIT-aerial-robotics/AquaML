...
import numpy as np
if TYPE_CHECKING:
    from AquaML.data.core_units import UnitConfig as unitCfg
else:
    from AquaML.data.core_units import UnitConfig as unitCfg
...
class GymnasiumWrapper(BaseEnv):
    ...
    def step(self, action: dict[str, np.ndarray]) -> tuple[dict[str, Any], Any, bool, bool, Any]:
        ...
        # Remove the batch dimensions for single environment
        if len(action_array.shape) >= 2:
            action_array = action_array[0, 0]

        next_observation, reward, done, truncated, info = self.env.step(action_array)

        if done or truncated:
            next_observation, info = self.env.reset()

        next_observation = np.expand_dims(next_observation, axis=[0, 1])
        reward = np.expand_dims(np.array([reward]), axis=[0, 1])
        done = np.expand_dims(np.array([done]), axis=[0, 1])
        truncated = np.expand_dims(np.array([truncated]), axis=[0, 1])
