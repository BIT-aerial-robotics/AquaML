...
class  DeterminateRealWorldPolicy(PolicyBase):
    ...
    def get_weight_path(self):
        ...
        # 得到完整的路径
        weight_path = os.path.join(self._history_model_path, str(history_model_serial_number))
        weight_path = os.path.join(weight_path, 'actor.h5')
        
        self._communicator.debug_info('RealWorldPolicyBase: {} get weight path:{}'.format(self._name_scope,weight_path))
    ...
    def load_weight_thread(self):
        ...
        while True:
            ...
            if self._switch_flag == 1:
                ...
                self._communicator.debug_info('RealWorldPolicyBase','Load actor2 weights')
                
                self._load_complete_flag = True
            else:
                self._actor1.load_weights(weight_path)
                self._communicator.debug_info('RealWorldPolicyBase','Load actor1 weights')
    ...
    def predict_action_thread(self):
        ...
        while True:
            input_data = self.get_robot_input_state()
            
            if self._switch_flag == 1:
                action = self._actor1(*input_data)
                self._communicator.debug_info('RealWorldPolicyBase','actor1 predict action')
            else:
                action = self._actor2(*input_data)
                self._communicator.debug_info('RealWorldPolicyBase','actor2 predict action')
            ...
            if self._total_interaction_times % self._switch_times == 0:
    ...
    def run(self):
        ...
        self._communicator.logger_info('RealWorldPolicyBase','Init thread.')
        ...
        self._communicator.logger_info('RealWorldPolicyBase','Start thread.')
