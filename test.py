...
class Evaluator(BaseWorker):
    ...
    def step(self, env, agent, collector):
        ...
        compute_obs = deepcopy(self.obs)
        for obs_plugin in self._obs_plugin:
            ob = obs_plugin(deepcopy(compute_obs), False)
            compute_obs.update(ob)

        action_dict = agent.get_action(compute_obs, test_flag=True)
...
class RLDistributedVectorEnvWorker(BaseWorker):
    """
